...
;; (defvar redspace-check-buffer-regex "[ +|\t+]")
(defvar redspace-check-buffer-regex "[ \t+]")
...
;; Modified to set check regex
(defun redspace-check-buffer()
  (interactive)
  (redspace-delete-own-overlays)
(save-excursion
  (goto-char (point-min))
  (while (re-search-forward (concat redspace-check-buffer-regex "$") (point-max) t)
      (let (e)
	(setq e (point))
	(while (looking-back redspace-check-buffer-regex)
	  (backward-char)
	  )
